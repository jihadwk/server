package com.wk.loginserver.service.channel;
...
import com.wk.exception.ManagerServerException;
import com.wk.loginserver.baseInfo.Dao.DBBaseInforDAO;
import com.wk.net.IService;
/**
 * @author zhaoqingyou
 *
 */
@Service("channelManager")
public class ChannelManagerImpl implements ChannelManager, IService {
...
//			this.channelInfoMap = new ConcurrentSkipListMap<Integer, CityServerChannleVO>();
	...
	public void registerChannelInfo() throws ManagerServerException {
		List<BwGameChannleVO> channleList =dbBaseInforDAO.getGameServerChannleList();
		//此地方需要获取服务器列表
		if (channleList == null) {
...
//		}
		...
		//获取文件服务器列表
		List<BwFileServerVO> fileServerList=dbBaseInforDAO.queryBwFileServerVO();
...
//			wGgameChannleVO.setUserCount(currentRole);
 ...
 public void initAreaList(){
		//初始化区域信息
		List<BwAreaVO> tempAreaVOList=dbBaseInforDAO.queryBwAreaVOList();
		if(null!=tempAreaVOList&&tempAreaVOList.size()>0){
			for(BwAreaVO arvo:tempAreaVOList){
				//根据区域iD获取区域服务器列表
				List<BwGameChannleVO> cityServerChannleList=dbBaseInforDAO.getGameServerChannleList(arvo.getAreaId());
